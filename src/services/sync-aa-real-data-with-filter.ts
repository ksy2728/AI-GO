#!/usr/bin/env tsx
/**
 * Enhanced AA Data Sync Script with Performance Filtering
 * Fetches data from Artificial Analysis and filters out below-average models
 */
...
const ENABLE_PERFORMANCE_FILTER = process.env.AA_ENABLE_PERFORMANCE_FILTER !== 'false'
const CUSTOM_MIN_INTELLIGENCE = process.env.AA_MIN_INTELLIGENCE ?
...
function filterModelsByPerformance(models: AAModel[]): AAModel[] {
  ...
  if (CUSTOM_MIN_INTELLIGENCE !== null) {
    threshold = CUSTOM_MIN_INTELLIGENCE
  ...
  if (removedCount > 0) {
    // Show some examples of filtered models
    const removedModels = models
      .filter(m => m.intelligenceScore < threshold)
      .slice(0, 5)
    console.log(`  - Examples of filtered models:`)
    removedModels.forEach(m => {
      console.log(`    • ${m.name} (Score: ${m.intelligenceScore.toFixed(2)})`)
    })
  }
...
/**
 * Main sync function with performance filtering
 */
async function syncAAData() {
  try {
    console.log('🚀 Starting AA data sync with performance filtering...')
    console.log('⚙️ Configuration:')
    console.log(`  - Performance filter: ${ENABLE_PERFORMANCE_FILTER ? 'ENABLED' : 'DISABLED'}`)
    if (CUSTOM_MIN_INTELLIGENCE !== null) {
      console.log(`  - Custom threshold: ${CUSTOM_MIN_INTELLIGENCE}`)
    } else {
      console.log(`  - Threshold: Dynamic (average)`)
    }
    console.log('')
    ...
    aaModels.slice(0, 5).forEach(m => {
      console.log(`  - ${m.name}: Score=${m.intelligenceScore.toFixed(2)}, Speed=${m.outputSpeed}`)
    })
